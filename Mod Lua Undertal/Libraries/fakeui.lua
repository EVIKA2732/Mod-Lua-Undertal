...
fui_var_buttonsprites = {"fakeui/buttons/", "fakeui/buttons/act_", "fakeui/buttons/", "fakeui/buttons/"}
fui_var_buttonsprites1_1 = {"fakeui/buttons/fightb1_", "fakeui/buttons/act_", "fakeui/buttons/", "fakeui/buttons/"}
fui_var_buttonsprites1_2 = {"fakeui/buttons/", "fakeui/buttons/act_", "fakeui/buttons/itemb_", "fakeui/buttons/"}
fui_var_buttonsprites3 = {"fakeui/buttons/fightb1_", "fakeui/buttons/act_", "fakeui/buttons/itemb_", "fakeui/buttons/"}
...
function fui_initButtons()
	for i=1,4 do
                                           if GetGlobal("bfight") >= 5 and GetGlobal("bitem") >= 4 then
             		    fui_buttons[i] = CreateSprite(fui_var_buttonsprites3[i] .. "0")
                                           elseif GetGlobal("bfight") >= 5 then
		    fui_buttons[i] = CreateSprite(fui_var_buttonsprites1_1[i] .. "0")
                                           elseif GetGlobal("bitem") >= 4 then
		    fui_buttons[i] = CreateSprite(fui_var_buttonsprites1_2[i] .. "0")
                                           else
...
function fui_update()
		--if(Player.absx == 202 and Player.absy == 25) then
	                        --     fui_fakePlayer.Set("ut-heart2")
                                           --end
	fui_updateHP()
	fui_updateName()
	fui_updateLV()
...
function fui_updateButtons()
	...
	if(fui_selectingButton and fui_var_selectedbtn ~= 0 and fui_fakePlayer == nil) then
		...
		if (Player.absx == 202 and Player.absy == 25 and GetGlobal("bfight") < 5) then
	                             fui_fakePlayer = CreateSprite("ut-heart2")
                                           elseif (GetGlobal("bfight") >= 5) then
	                             fui_fakePlayer = CreateSprite("ut-heart2")
                                           elseif (GetGlobal("bitem") >= 4) then
	                             fui_fakePlayer = CreateSprite("ut-heart2")
	...
	if(fui_var_selectedbtn ~= fui_var_lastselected) then
		for i=1,#fui_buttons do
			if(fui_var_selectedbtn == i) then
                                                                               if GetGlobal("bfight") >= 5 and GetGlobal("bitem") >= 4 then
				fui_buttons[i].Set(fui_var_buttonsprites3[i] .. "1")
                                                                               elseif GetGlobal("bfight") >= 5 then
				fui_buttons[i].Set(fui_var_buttonsprites1_1[i] .. "1")
                                                                               elseif GetGlobal("bitem") >= 4 then
				fui_buttons[i].Set(fui_var_buttonsprites1_2[i] .. "1")
                                                                               else
				fui_buttons[i].Set(fui_var_buttonsprites[i] .. "1")
                                                                               end
			else
                                                                               if GetGlobal("bfight") >= 5 and GetGlobal("bitem") >= 4 then
				fui_buttons[i].Set(fui_var_buttonsprites3[i] .. "0")
                                                                               elseif GetGlobal("bfight") >= 5 then
				fui_buttons[i].Set(fui_var_buttonsprites1_1[i] .. "0")
                                                                               elseif GetGlobal("bitem") >= 4 then
				fui_buttons[i].Set(fui_var_buttonsprites1_2[i] .. "0")
                                                                               else
