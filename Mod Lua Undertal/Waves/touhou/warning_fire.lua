...
emitter_bullet_yvel = 155 -- pixels per second traveled by emitter bullets
emitter_bullet_amplitude = ARENA_WIDTH * 0.4
emitter_bullet_period = 66 -- time for bullets to do a complete wave in frames


time_to_first_warning = 30 --expected frames for first warning to show up
warning_duration = 20 -- expected frames from when a warning box appears to when the fire streams start
warning_horizontal = 85
warning_vertical = 187
...
function create_warning()
                     if math.random(1,3) == 1 or Input.Cancel == 2 then
		offset = Player.x
		exclam_offset = Player.x
	elseif Player.x < -30 then
		offset = -Arena.width/2 + warning_horizontal / 2 + 1
		exclam_offset = -Arena.width / 2 + 44
	elseif Player.x > 30 then
		offset = Arena.width/2 - warning_horizontal / 2 - 1
		exclam_offset = Arena.width / 2 - 44
