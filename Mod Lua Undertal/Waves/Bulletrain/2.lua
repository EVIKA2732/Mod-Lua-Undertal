...
function Update()
	...
	if (currenttime > bulletspawner and currentwave <= bulletwaves) then
		...
		for i=1,bulletcount do
			...
			SpawnBulletAbs(Time.time, bulletwavex, 0, (360/bulletcount) * i)
			SpawnBulletAbs(Time.time, bulletwavex1, 0, (360/bulletcount) * i)
		end
...
function SpawnBulletAbs(key, x, y, ang)
	local bullet = CreateProjectileAbs("bullet/small_fire_collision", x, y)
